...
def psignup():
    if (request.method == 'POST'):
        ...
        ext=str(random.randint(1,9))+str(random.randint(1,9))+str(random.randint(1,9))+str(random.randint(1,9))
        entry = patient(pid=state[:2]+ext,infection='0',book_stat=0,adharname=name,password=pas,adharno=adhar,father=fname,mother=mname,pincode=pin,gender=gen,dob=dob, email=email, contact=phone, state=state,district=city,address=addr)
...
def lsignup():
    if (request.method == 'POST'):
        ...
        pin = request.form.get('pincode')
        entry = sample_collection(regno=reg_no,regname=name,password=pas,state=state,district=city,pincode=pin,address=addr,email=email,contact=phone,worktstart=start,worktend=end)
        db.session.add(entry)
        db.session.commit()
        flash("Successfully Signed UP")
